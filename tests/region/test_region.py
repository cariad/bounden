...
# RegionType = Region[tuple[str, int], tuple[LengthC[int], LengthC[int]]]


# @fixture
# def region() -> RegionType:
#     return RegionType(("A", 1), (LengthC(3), LengthC(7)))
...
# def test_eq() -> None:
#     assert Region((1, 2), (LengthC(3), LengthC(4))) == Region(
#         (1, 2), (LengthC(3), LengthC(4))
#     )
...
def test_resolve__far() -> None:
    parent = Region(
        (IntegerAxisPosition(2), IntegerAxisPosition(3)),
        (LengthC(7), LengthC(15)),
    )
    child = parent.region((Alignment.Far, 4), (LengthC(3), LengthC(9)))

    expect_point = ResolvedPoint(
        (IntegerAxisPosition(6), IntegerAxisPosition(7))
    )

    assert child.resolve() == (
        expect_point,
        ResolvedVolume((LengthC(3), LengthC(9))),
    )


# pylint: disable-next=redefined-outer-name
# def test_size(region: RegionType) -> None:
#     assert region.volume == (LengthC(3), LengthC(7))
